...
#filtered path - This is the home cage multimouse data, morphologically filtered and dilated as described in sections 3.11 and 5.4 of the manuscript
...
# Median function for mousevidscleaned dataframes, only takes median of relevant columns and leaves metadata columns alone
median_ignoremetadata <- function(file){
...
for (i in 1:length(mouse1_vids_cleaned)){
  ...
  mouse1_vidmedians[[i]] <- median_ignoremetadata(mouse1_vids_cleaned[[i]])
...
for (i in 1:length(mouse2_vids_cleaned)){
  ...
  mouse2_vidmedians[[i]] <- median_ignoremetadata(mouse2_vids_cleaned[[i]])
...
for (i in 1:length(mouse3_vids_cleaned)){
  ...
  mouse3_vidmedians[[i]] <- median_ignoremetadata(mouse3_vids_cleaned[[i]])
