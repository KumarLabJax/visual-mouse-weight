#!/bin/bash
#
#SBATCH --job-name=master_pixel_analysis
#SBATCH --time=6:00:00
#SBATCH --ntasks=1
...
then
    ...
    then
        ...
        then
            ...
            MOMENT_FILE=$(trim_sp $(sed -n "${SLURM_ARRAY_TASK_ID}{p;q;}" < "${VIDEO_BATCH}"))
            FULL_MOMENT_FILE="${MOMENT_FILE}"
            if [[ -f "${FULL_MOMENT_FILE}" ]]
            then
                echo "BEGIN TRACKING VIDEO FILE: ${FULL_MOMENT_FILE}"

                module load singularity

                #Assumes existence of csvs
                singularity exec --nv "/projects/kumar-lab/guzmam/environment/UPennInferImg.simg" /bin/bash -c "python3 /projects/kumar-lab/guzmam/fullSurvey/code/moment_analysis.py ${FULL_MOMENT_FILE}"

                echo "FINISHED PROCESSING VIDEO FILE: ${FULL_MOMENT_FILE}"
            else
                echo "ERROR: could not find video file: ${FULL_MOMENT_FILE}" >&2
            fi
        else
